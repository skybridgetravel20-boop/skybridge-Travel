...
import { downloadCrmDataAsSheet, createGoogleCalendarUrl, createGmailComposeUrl } from '../../lib/googleWorkspace';
...
export const GoogleWorkspaceHub: React.FC = () => {
  ...
  // Email state
  const [emailTo, setEmailTo] = useState('skybridgetravel20@gmail.com');
  const [emailSubject, setEmailSubject] = useState('SkyBridge VIP Travel Confirmation - Dossier Update');
  const [emailBody, setEmailBody] = useState('Dear Valued Traveler,\n\nWe are pleased to inform you that your travel booking dossier has been updated by SkyBridge Travel & Tourism.\n\nWarm regards,\nSaman, CEO\nSkyBridge Travel & Tourism');
        ...
        {activeTab === 'gmail' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <Mail className="w-5 h-5 text-rose-400" /> Gmail Executive Dispatch
                </h3>
                <p className="text-slate-400 text-xs mt-0.5">
                  Send official emails and notifications directly via your official account: <span className="text-rose-300 font-mono">skybridgetravel20@gmail.com</span>.
                </p>
              </div>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-3.5 py-1.5 rounded-lg bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/30 text-xs font-medium flex items-center gap-1.5"
              >
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">To Email</label>
                <input
                  type="email"
                  value={emailTo}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => {
                    window.open(createGmailComposeUrl({ to: emailTo, subject: emailSubject, body: emailBody }), '_blank');
                  }}
                  className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs transition-colors flex items-center gap-2 cursor-pointer shadow-lg shadow-rose-600/20"
                >
                  <Send className="w-3.5 h-3.5" /> Dispatch via Gmail API
                </button>
              </div>
            </div>
          </div>
